...
def create_test_audio(output_path="test-assets/test_audio.wav", total_duration=180):
    ...
    for i, bpm in enumerate(bpm_sequence):
        ...
        background = generate_background_tone(bg_frequencies[i], segment_duration, sample_rate)
        ...
        print(f"Segment {i+1} complete: {len(segment)} samples, duration: {len(segment)/sample_rate:.1f}s")
    ...
    print(f"Duration: {len(final_audio_int)/sample_rate:.1f} seconds ({len(final_audio_int)/sample_rate/60:.1f} minutes)")
