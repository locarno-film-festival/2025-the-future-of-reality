...
class TestPerformanceBenchmarks:
    ...
    def test_audio_analysis_performance(self, test_audio_path):
        ...
        assert (
            beat_detection_time < 10.0
        ), f"Beat detection took {beat_detection_time:.2f}s, should be < 10s"

        # Memory usage check
        audio_size_mb = len(y) * 4 / (1024 * 1024)  # 4 bytes per float32
        assert (
            audio_size_mb < 100
        ), f"Audio data using {audio_size_mb:.1f}MB, should be < 100MB"
    ...
    def test_scene_detection_performance(self, long_test_video_path):
        ...
        assert (
            detection_time < 60.0
        ), f"Scene detection took {detection_time:.2f}s, should be < 60s"
        ...
        assert (
            scenes_per_processing_second > 5
        ), f"Processing rate: {scenes_per_processing_second:.1f} scenes/sec"
    ...
    def test_file_io_performance(self, temp_output_dir):
        ...
        large_data = {
            "scenes": [
                {"start": i, "end": i + 1, "duration": 1.0} for i in range(1000)
            ],
...
class TestScalabilityBenchmarks:
    ...
    def test_video_length_scaling(self):
        ...
        for i, ratio in enumerate(expected_processing_ratios):
            assert (
                ratio < 0.1
            ), f"Processing ratio for {video_lengths[i]}s video: {ratio:.3f}"
    ...
    def test_scene_count_scaling(self):
        ...
        for count in scene_counts:
            ...
            assert (
                estimated_time < 120
            ), f"Estimated time for {count} scenes: {estimated_time:.1f}s"
