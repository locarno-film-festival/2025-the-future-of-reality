#!/usr/bin/env python3
"""
ULTRA-ROBUST ARCHIVAL ANALYSIS & REMIX ENGINE v22
Fixed audio handling - all clips should have audio when source has audio
...
class UltraRobustArchivalEngine:
    """
    Ultra-robust version with fixed audio support - all clips get audio when source has audio.
    """
    ...
    def export_scene_clip_with_audio_fixed(self, video, scene_info, max_duration=1000):
        """
        Fixed audio export - tries multiple methods to ensure audio inclusion.
        """
        ...
        try:
            ...
            if self.has_audio and video.audio is not None:
                # Method 1: Direct export with audio (most reliable)
                try:
                ...
                try:
                    print(f"     Trying alternative audio settings for scene {scene_info['id']}...")
                    clip.write_videofile(
                        str(clip_path),
                        codec='libx264',
                        audio=True,
                        audio_codec='mp3',  # Try MP3 instead of AAC
                        verbose=False,
                        logger=None,
                        preset='ultrafast',
                        threads=1,
                    ...
                    scene_info['has_audio'] = True
                    print(f"     ✓ Scene {scene_info['id']} exported with audio (alternative settings)")
                    clip.close()
                    return True
                    
                except Exception as alt_error:
                    print(f"     ✗ Alternative audio settings failed for scene {scene_info['id']}: {str(alt_error)[:50]}")
...
<head>
    ...
    <title>Interactive Archival Film Analysis v22</title>
...
<body>
    <div class="container">
        <h1>🎬 Interactive Archival Film Analysis v22</h1>
        
        <div class="metadata">
            <strong>Output Directory:</strong> ''' + str(self.output_dir.name) + '''<br>
            <strong>Timestamp:</strong> ''' + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + '''<br>
            <strong>Audio Support:</strong> Fixed - ALL clips should have audio when source has audio<br>
    ...
    <script>
        ...
        print(f"   ✓ Fixed audio support indicators")
    ...
    def run_complete_pipeline(self, export_clips=True, max_scenes=None, 
        ...
        """
        Run complete analysis and remix pipeline with fixed audio support.
        """
        print(f"""
        ╔══════════════════════════════════════════════════════════════╗
        ║  STARTING ARCHIVAL TRANSFORMATION v22                       ║
        ╠══════════════════════════════════════════════════════════════╣
        ...
        ║  🔊 Fixed audio support - ALL clips should have audio       ║
        ╚══════════════════════════════════════════════════════════════╝
        """)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("""
        ULTRA-ROBUST ARCHIVAL ENGINE v22 - Fixed Audio Support - Usage:
        ...
        5. Include audio in exported clips (FIXED):
        ...
        v22 Features:
        - 🔧 FIXED: All clips now have audio when source has audio
        - 🎵 Multiple export methods with better fallback handling
        - 🛡️ Robust error recovery prevents premature audio abandonment
        - 📊 Accurate audio indicators in HTML visualization
        - 🎬 Smart audio/video combination using multiple methods
    ...
    success = engine.run_complete_pipeline(
        ...
        with_audio=with_audio  # Pass the fixed audio flag
    )
