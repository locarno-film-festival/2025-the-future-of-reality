#!/usr/bin/env python3
"""
ULTRA-ROBUST ARCHIVAL ANALYSIS & REMIX ENGINE v21
Enhanced with improved audio support for subclips
...
class UltraRobustArchivalEngine:
    """
    Ultra-robust version with enhanced audio support for subclips.
    """
    ...
    def export_scene_clip_with_audio(self, video, scene_info, max_duration=1000, force_audio=False):
        """
        Export a single scene as a video clip with enhanced audio handling.
        
        Args:
            video: The source video clip
            scene_info: Scene metadata dictionary
            max_duration: Maximum clip duration in seconds
            force_audio: If True, tries harder to include audio even if initial check fails
        """
        ...
        try:
            ...
            include_audio = False
            if self.has_audio or force_audio:
                if video.audio is not None:
                    # Check if audio track actually has data in this range
                    try:
                        audio_clip = clip.audio
                        if audio_clip is not None:
                            # Test if we can get audio data
                            test_audio = audio_clip.to_soundarray(fps=22050, nbytes=2, quantize=True)
                            if test_audio.size > 0:
                                include_audio = True
                                print(f"     ✓ Audio included for scene {scene_info['id']}")
                            else:
                                print(f"     ⚠ Empty audio track for scene {scene_info['id']}")
                    except Exception as audio_test_error:
                        print(f"     ⚠ Audio test failed for scene {scene_info['id']}: {str(audio_test_error)[:30]}")
            
            # Export with appropriate settings
            if include_audio:
                try:
                    # Method 1: Direct export with audio
                    clip.write_videofile(
                        ...
                        fps=24,
                        write_logfile=False,
                        temp_audiofile=str(self.temp_dir / f"temp_audio_{scene_info['id']}.wav")
                    )
                    scene_info['has_audio'] = True
                    
                except Exception as direct_export_error:
                    print(f"     Direct audio export failed for scene {scene_info['id']}: {str(direct_export_error)[:50]}")
                    
                    try:
                        # Method 2: FFmpeg combination approach
                        ...
                        if clip.audio is not None:
                            ...
                            if success:
                                scene_info['has_audio'] = True
                                print(f"     ✓ Audio combined via FFmpeg for scene {scene_info['id']}")
                            else:
                                # Fallback to video-only
                                print(f"     ⚠ FFmpeg combination failed, using video-only for scene {scene_info['id']}")
                                if temp_video_path.exists():
                                    temp_video_path.rename(clip_path)
                        ...
                        # Final fallback to video-only
                        self._export_video_only(clip, clip_path, scene_info)
            ...
            error_msg = str(e)
            print(f"     Export failed for scene {scene_info['id']}: {error_msg[:50]}")
    ...
    def _export_video_only(self, clip, clip_path, scene_info):
        """Fallback method to export video without audio."""
        try:
            clip.write_videofile(
                str(clip_path),
                codec='libx264',
            ...
            scene_info['has_audio'] = False
            print(f"     ✓ Video-only export for scene {scene_info['id']}")
            return True
        except Exception as e:
    ...
    def export_scene_clip(self, video, scene_info, max_duration=1000, with_audio=False):
        ...
        Routes to the enhanced audio export method when audio is requested.
        """
        if with_audio:
            return self.export_scene_clip_with_audio(video, scene_info, max_duration, force_audio=True)
        else:
            return self._export_video_only(video.subclip(scene_info['start'], 
                                                        min(scene_info['end'], scene_info['start'] + max_duration)), 
                                          self.clips_dir / scene_info['clip_filename'], 
                                          scene_info)
    ...
    def analyze_and_export_scenes(self, threshold=30.0, export_clips=True, max_scenes=None, max_clip_duration=1000, with_audio=False):
        """
        Analyze scenes with enhanced clip export for interactive playback.
        """
        print("\n📽️  SCENE ANALYSIS...")
...
<head>
    ...
    <title>Interactive Archival Film Analysis v21</title>
...
<body>
    ...
    <script>
        ...
        print(f"   ✓ Enhanced with audio support indicators")
    ...
    def run_complete_pipeline(self, export_clips=True, max_scenes=None, 
        ...
        """
        Run complete analysis and remix pipeline with enhanced audio support.
        """
        print(f"""
        ╔══════════════════════════════════════════════════════════════╗
        ║  STARTING ARCHIVAL TRANSFORMATION v21                       ║
        ╠══════════════════════════════════════════════════════════════╣
        ...
        ║  🔊 Enhanced audio support for subclips                     ║
        ╚══════════════════════════════════════════════════════════════╝
        """)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("""
        ULTRA-ROBUST ARCHIVAL ENGINE v21 - Enhanced Audio Support - Usage:
        ...
        5. Include audio in exported clips (ENHANCED):
        ...
        - 🎵 Improved audio track detection and validation
        - 🔊 Better error handling for audio export failures
        - 📊 Audio indicators in HTML visualization
        - 🎬 Smart audio/video combination using FFmpeg
        - 🛡️ Robust error recovery for audio processing
    ...
    success = engine.run_complete_pipeline(
        ...
        with_audio=with_audio  # Pass the enhanced audio flag
    )
