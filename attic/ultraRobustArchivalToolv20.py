...
class UltraRobustArchivalEngine:
    ...
    def export_scene_clip(self, video, scene_info, max_duration=1000, with_audio=False):
        ...
        try:
            ...
            # For audio clips, we need to ensure audio is properly initialized
            if with_audio and video.audio is not None:
                # Try to write with specific audio parameters
                try:
                    # Use a temporary audio file with specific format
                    temp_audiofile = str(self.temp_dir / f"temp_audio_{scene_info['id']}.m4a")
                    ...
                    clip.write_videofile(
                        ...
                        remove_temp=True,
                        write_logfile=False  # Disable log file to avoid stdout issues
                    )
                    clip.close()
                    return True
                    
                except Exception as audio_error:
                    # If audio export fails, try without audio
                    print(f"     Audio export failed for scene {scene_info['id']}: {str(audio_error)[:30]}")
                    print(f"       Retrying without audio...")
                    
                    # Close and recreate clip
                    try:
                        clip.close()
                    except:
                        pass
            ...
            if 'NoneType' in error_msg and 'stdout' in error_msg:
                print(f"     MoviePy subprocess error for scene {scene_info['id']} ({start_time:.2f}-{end_time:.2f}s)")
            else:
    ...
    def analyze_and_export_scenes(self, threshold=30.0, export_clips=True, max_scenes=None, max_clip_duration=1000, with_audio=False):
        ...
        try:
            # Load video - include audio if requested for clips
            if with_audio and export_clips:
                print("   Loading video with audio for clip export...")
                try:
                    video = VideoFileClip(self.film_path, audio=True)
                    # Test if audio is actually available
                    if video.audio is None:
                        print("   Note: No audio track found in video, continuing without audio...")
                        with_audio = False
                except Exception as audio_error:
            ...
            self.scene_data = []
            clips_exported = 0
            clips_failed = []  # Track which clips fail
            ...
            if export_clips:
                ...
                if clips_failed:
                    ...
                    if len(clips_failed) > 0 and len(clips_failed) < 50:
                        print(f"   ⚠ Failed clips: {clips_failed[:20]}...")  # Show first 20
                    
                    # Check if there's a pattern (every other clip fails?)
                    if len(clips_failed) > 5:
                        # Check if it's alternating
                        diffs = [clips_failed[i+1] - clips_failed[i] for i in range(min(10, len(clips_failed)-1))]
                        if diffs and all(d == diffs[0] for d in diffs):
                            print(f"   ⚠ Pattern detected: Failures occur every {diffs[0]} clips")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        5. Include audio in exported clips:
        ...
        - 🔊 Optional audio in exported scene clips
        """)
        return
    ...
    success = engine.run_complete_pipeline(
        ...
        with_audio=with_audio  # Pass the audio flag
    )
